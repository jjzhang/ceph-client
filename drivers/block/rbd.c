...
static void rbd_watch_cb(u64 ver, u64 notify_id, u8 opcode, void *data)
{
	...
	header_ver = dev->header.obj_version;
	mutex_unlock(&ctl_mutex);
	if (rc)
		pr_warning(DRV_NAME "%d got notification but failed to update"
			   " snaps: %d\n", dev->major, rc);

	rbd_req_sync_notify_ack(dev, header_ver, notify_id, dev->obj_md_name);
}
...
next_seg:
		...
		spin_lock_irq(q->queue_lock);
next_snapc:
		/* osd_client has it's own reference
		   to snapc - we don't need it anymore */
...
static int __rbd_refresh_header(struct rbd_device *rbd_dev)
{
	...
	if (rbd_dev->snap_id == CEPH_NOSNAP) {
		dout("setting size to %llu sectors", h.image_size / 512ULL);
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%llu\n", (unsigned long long) size * 512ULL);
}
